  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--ink)',color:'var(--cream)'}}>
      <div style={{background:'var(--ink-2)',borderBottom:'1px solid var(--border-2)',padding:'0 32px',display:'flex',alignItems:'center',gap:32}}>
        ...
        <a href="/community" style={{fontSize:15,color:'var(--muted)',textDecoration:'none'}}>← Back to portal</a>
      </div>
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search users…" style={{background:'var(--ink-2)',border:'1px solid var(--border-2)',borderRadius:8,padding:'8px 14px',color:'var(--cream)',fontSize:15,outline:'none',width:240}} />
      </div>
      <div style={{background:'var(--ink-2)',borderRadius:12,overflow:'hidden',border:'1px solid var(--border-2)'}}>
        <div style={{display:'grid',gridTemplateColumns:'2fr 2fr 1.5fr 1fr 80px 100px 100px 200px',padding:'10px 16px',background:'var(--ink)',fontSize:13,fontWeight:600,color:'var(--muted)',textTransform:'none',letterSpacing:'0.01em'}}>
        ...
        {filtered.map((u,i)=>(
          <div key={u.id} style={{display:'grid',gridTemplateColumns:'2fr 2fr 1.5fr 1fr 80px 100px 100px 200px',padding:'12px 16px',borderTop:'1px solid var(--border-2)',background:i%2===0?'transparent':'rgba(255,255,255,0.01)',alignItems:'center'}}>
            <div style={{color:'var(--cream)',fontSize:15,fontWeight:500}}>{u.name}</div>
            <div style={{color:'var(--muted)',fontSize:14}}>{u.email}</div>
            <div style={{color:'var(--muted)',fontSize:14}}>{u.company||'—'}</div>
            <div style={{color:'var(--muted)',fontSize:15}}>{u.assessment_count||0}</div>
            <div style={{color:u.status==='active'?'var(--green)':'var(--red)',fontSize:14,fontWeight:600}}>{u.status}</div>
            <div style={{color:u.is_admin?'var(--gold)':'var(--muted)',fontSize:14,fontWeight:600}}>{u.is_admin?'Admin':'Member'}</div>
            <div style={{color:'var(--muted)',fontSize:14}}>{u.chat_month||0} msgs</div>
...
function WhitepapersTab() {
  ...
  if (editing) return (
    <div>
      <button onClick={()=>setEditing(null)} style={{background:'none',border:'1px solid var(--border-2)',borderRadius:8,padding:'8px 14px',color:'var(--muted)',cursor:'pointer',fontSize:15,marginBottom:20}}>← Cancel</button>
      <div style={{background:'var(--ink-2)',border:'1px solid var(--border-2)',borderRadius:16,padding:24,display:'grid',gap:16}}>
        {[['Title','title','input'],['Category','category','input'],['Description','description','textarea'],['Icon (emoji)','icon','input'],['Pages','pages','input'],['File URL','file_url','input']].map(([label,field,type])=>(
          <div key={field as string}>
            <div style={{fontSize:14,color:'var(--muted)',marginBottom:6,fontWeight:600,textTransform:'none',letterSpacing:'0.01em'}}>{label as string}</div>
            {type==='textarea' ? <textarea value={editing[field as string]||''} onChange={e=>setEditing((p:any)=>({...p,[field as string]:e.target.value}))} rows={3} style={{width:'100%',background:'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,padding:'10px 14px',color:'var(--cream)',fontSize:15,outline:'none',resize:'vertical',boxSizing:'border-box'}} /> : <input value={editing[field as string]||''} onChange={e=>setEditing((p:any)=>({...p,[field as string]:e.target.value}))} style={{width:'100%',background:'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,padding:'10px 14px',color:'var(--cream)',fontSize:15,outline:'none',boxSizing:'border-box'}} />}
          </div>
        ))}
        <div>
          <div style={{fontSize:14,color:'var(--muted)',marginBottom:6,fontWeight:600,textTransform:'none',letterSpacing:'0.01em'}}>Access</div>
          <select value={editing.access||'members'} onChange={e=>setEditing((p:any)=>({...p,access:e.target.value}))} style={{background:'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,padding:'10px 14px',color:'var(--cream)',fontSize:15,cursor:'pointer',outline:'none'}}>
            <option value="public">Public</option><option value="members">Members only</option>
          </select>
        </div>
        <button onClick={save} style={{background:'var(--cream)',border:'none',borderRadius:8,padding:'12px 24px',color:'var(--gold-btn-text)',fontWeight:700,cursor:'pointer',fontSize:16}}>Save</button>
      </div>
    </div>
  );
  return (
      ...
      <div style={{display:'grid',gap:12}}>
        {papers.map(p=>(
          <div key={p.id} style={{background:'var(--ink-2)',border:'1px solid var(--border-2)',borderRadius:12,padding:'16px 20px',display:'flex',alignItems:'center',gap:16}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:700,color:'var(--cream)'}}>{p.title}</div><div style={{fontSize:14,color:'var(--muted)'}}>{p.category} · {p.pages}pp · {p.access}</div></div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setEditing(p)} style={{background:'none',border:'1px solid var(--border-2)',borderRadius:8,padding:'6px 14px',color:'var(--muted)',cursor:'pointer',fontSize:14}}>Edit</button>
              <button onClick={()=>delPaper(p.id)} style={{background:'none',border:'1px solid var(--red)',borderRadius:8,padding:'6px 14px',color:'var(--red)',cursor:'pointer',fontSize:14}}>Delete</button>
            </div>
          </div>
        ))}
      </div>
...
function ChatConfigTab() {
  ...
  return (
    <div>
      ...
      <p style={{color:'var(--muted)',fontSize:15,marginBottom:28}}>Configure the AI chatbot that allows members to query The Pivot Model book. API key is stored in the database.</p>
      <div style={{background:'var(--ink-2)',border:'1px solid var(--border-2)',borderRadius:16,padding:28,maxWidth:640}}>
        <div style={{marginBottom:20}}>
          <div style={{fontSize:14,color:'var(--muted)',marginBottom:6,fontWeight:600,textTransform:'none',letterSpacing:'0.01em'}}>Chat Enabled</div>
          <div style={{display:'flex',gap:8}}>
            {['true','false'].map(v=><button key={v} onClick={()=>setConfig(p=>({...p,chat_enabled:v}))} style={{padding:'8px 20px',background:config.chat_enabled===v?'var(--gold)':'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,color:config.chat_enabled===v?'var(--gold-btn-text)':'var(--muted)',fontWeight:config.chat_enabled===v?700:400,cursor:'pointer',fontSize:15}}>{v==='true'?'Enabled':'Disabled'}</button>)}
          </div>
        </div>
        <div style={{marginBottom:20}}>
          <div style={{fontSize:14,color:'var(--muted)',marginBottom:6,fontWeight:600,textTransform:'none',letterSpacing:'0.01em'}}>Anthropic API Key</div>
          <input type="password" value={config.chat_api_key} onChange={e=>setConfig(p=>({...p,chat_api_key:e.target.value}))} placeholder="sk-ant-…" style={{width:'100%',background:'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,padding:'10px 14px',color:'var(--cream)',fontSize:15,outline:'none',fontFamily:'inherit',boxSizing:'border-box'}} />
          <div style={{fontSize:13,color:'var(--muted)',marginTop:4}}>Stored in the database. Never exposed to users or client-side code.</div>
        </div>
        <div style={{marginBottom:20}}>
          <div style={{fontSize:14,color:'var(--muted)',marginBottom:6,fontWeight:600,textTransform:'none',letterSpacing:'0.01em'}}>System Prompt (book context)</div>
          <textarea value={config.chat_system_prompt} onChange={e=>setConfig(p=>({...p,chat_system_prompt:e.target.value}))} rows={6} style={{width:'100%',background:'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,padding:'10px 14px',color:'var(--cream)',fontSize:15,outline:'none',resize:'vertical',boxSizing:'border-box'}} />
        </div>
        <div style={{marginBottom:24}}>
          <div style={{fontSize:14,color:'var(--muted)',marginBottom:12,fontWeight:600,textTransform:'none',letterSpacing:'0.01em'}}>Usage Limits (per user)</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:16}}>
            {[['Daily limit','chat_limit_day'],['Weekly limit','chat_limit_week'],['Monthly limit','chat_limit_month']].map(([label,field])=>(
              <div key={field}>
                ...
                <input type="number" value={(config as any)[field]} onChange={e=>setConfig(p=>({...p,[field]:e.target.value}))} style={{width:'100%',background:'var(--ink)',border:'1px solid var(--border-2)',borderRadius:8,padding:'8px 12px',color:'var(--cream)',fontSize:16,fontWeight:700,outline:'none',boxSizing:'border-box'}} />
              </div>
            ))}
          </div>
          <div style={{fontSize:13,color:'var(--muted)',marginTop:8}}>Limits are enforced server-side using rolling time windows. Changes take effect immediately without a redeploy.</div>
        </div>
        <button onClick={save} disabled={saving} style={{background:'var(--cream)',border:'none',borderRadius:8,padding:'12px 24px',color:'var(--gold-btn-text)',fontWeight:700,cursor:'pointer',fontSize:16,opacity:saving?0.7:1}}>
...
function btnSm(color:string) {
  return { background:'none' as const, border:`1px solid ${color}`, borderRadius:6, padding:'4px 10px', color, cursor:'pointer' as const, fontSize:13, fontWeight:600 as const };
}
