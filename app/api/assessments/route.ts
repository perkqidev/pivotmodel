...
import { query, queryOne, execute, EMB_TEMPLATE, BENCHMARK_TEMPLATE, SCOPE_TEMPLATE } from '@/lib/db';
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'duplicate' && assessment_id) {
    ...
    await execute(`INSERT INTO assessment_business_drivers (assessment_id,category,driver_name,description,current_state,target_state,priority,notes,sort_order) SELECT $1,category,driver_name,description,current_state,target_state,priority,notes,sort_order FROM assessment_business_drivers WHERE assessment_id=$2`, [nid, assessment_id]);
    await execute(`INSERT INTO assessment_benchmarks (assessment_id,pillar,kpi_name,unit,target_value,current_value,status,notes,sort_order) SELECT $1,pillar,kpi_name,unit,target_value,current_value,status,notes,sort_order FROM assessment_benchmarks WHERE assessment_id=$2`, [nid, assessment_id]);
    await execute(`INSERT INTO assessment_scope (assessment_id,pillar,activity,required_level,current_level,gap,notes,sort_order) SELECT $1,pillar,activity,required_level,current_level,gap,notes,sort_order FROM assessment_scope WHERE assessment_id=$2`, [nid, assessment_id]);
    await execute(`INSERT INTO assessment_kra (assessment_id,role_level,person_name,kra_name,description,target,current,status,notes,sort_order) SELECT $1,role_level,person_name,kra_name,description,target,current,status,notes,sort_order FROM assessment_kra WHERE assessment_id=$2`, [nid, assessment_id]);
    await execute(`INSERT INTO assessment_leadership (assessment_id,leader_name,leader_role,skill_name,is_mandatory,score,notes,sort_order) SELECT $1,leader_name,leader_role,skill_name,is_mandatory,score,notes,sort_order FROM assessment_leadership WHERE assessment_id=$2`, [nid, assessment_id]);
  ...
  for (const r of BENCHMARK_TEMPLATE) await execute(`INSERT INTO assessment_benchmarks (assessment_id,pillar,kpi_name,unit,target_value,sort_order) VALUES ($1,$2,$3,$4,$5,$6)`, [aid,r.pillar,r.kpi_name,r.unit,r.target_value,r.sort_order]);
  for (const r of SCOPE_TEMPLATE) await execute(`INSERT INTO assessment_scope (assessment_id,pillar,activity,sort_order) VALUES ($1,$2,$3,$4)`, [aid,r.pillar,r.activity,r.sort_order]);
