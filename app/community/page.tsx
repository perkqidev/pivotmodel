  ...
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24 }}>
        ...
        <button onClick={()=>setShowNew(o=>!o)} style={{ background:'var(--cream)',border:'none',borderRadius:8,padding:'10px 20px',color:'var(--gold-btn-text)',fontWeight:700,cursor:'pointer' }}>+ New Assessment</button>
      </div>
      {showNew && (
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:16,marginBottom:16 }}>
            {[['Team name','team_name','text','e.g. Platform Engineering'],['Industry','industry','text','e.g. FinTech'],['Date','assessment_date','date','']].map(([label,field,type,ph])=>(
              <div key={field as string}>
                ...
                <input type={type as string} value={(form as any)[field as string]} onChange={e=>setForm(p=>({...p,[field as string]:e.target.value}))} placeholder={ph as string} style={{ width:'100%',background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,padding:'10px 14px',color:'var(--fg)',fontSize:16,outline:'none',boxSizing:'border-box' }} />
              </div>
            ))}
          </div>
          <div style={{ display:'flex',gap:8 }}>
            <button onClick={createAssessment} disabled={creating||!form.team_name.trim()} style={{ background:'var(--cream)',border:'none',borderRadius:8,padding:'10px 24px',color:'var(--gold-btn-text)',fontWeight:700,cursor:'pointer',opacity:creating?0.7:1 }}>{creating?'Creating…':'Create Assessment'}</button>
...
function AssessmentList({ title, assessments, onDuplicate }: { title:string; assessments:Assessment[]; onDuplicate:(id:number)=>void }) {
  ...
  return (
    <div style={{ marginBottom:32 }}>
      <div style={{ fontWeight:600,color:'var(--muted)',fontSize:14,textTransform:'none',letterSpacing:'0.01em',marginBottom:12 }}>{title}</div>
      <div style={{ display:'grid',gap:12 }}>
        {assessments.map(a=>(
          <div key={a.id} style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between' }}>
            <div>
              <div style={{ fontWeight:700,color:'var(--fg)',fontSize:18 }}>{a.team_name}</div>
              <div style={{ color:'var(--muted)',fontSize:14,marginTop:2 }}>{a.industry||'—'} · {a.assessment_date?.slice(0,10)||'No date'} · Updated {new Date(a.updated_at).toLocaleDateString()}</div>
            </div>
            <div style={{ display:'flex',gap:8 }}>
              <button onClick={()=>onDuplicate(a.id)} style={{ background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,padding:'8px 14px',color:'var(--muted)',fontSize:14,cursor:'pointer' }}>Duplicate</button>
              <a href={`/assessment/${a.id}`} style={{ background:'var(--cream)',borderRadius:8,padding:'8px 16px',color:'var(--gold-btn-text)',fontWeight:700,fontSize:15,textDecoration:'none',display:'inline-block' }}>Open →</a>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:16 }}>
        {papers.map(p=>(
          <div key={p.id} style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:20 }}>
            ...
            <div style={{ color:'var(--muted)',fontSize:15,marginBottom:8 }}>{p.description}</div>
            <div style={{ fontSize:14,color:'var(--muted)' }}>{p.pages} pages · {p.access}</div>
            {p.file_url && <a href={p.file_url} target="_blank" style={{ display:'inline-block',marginTop:12,background:'var(--cream)',borderRadius:8,padding:'6px 14px',color:'var(--gold-btn-text)',fontWeight:700,fontSize:14,textDecoration:'none' }}>Download</a>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:24,maxWidth:400 }}>
        <div style={{ marginBottom:16 }}><div style={{ fontSize:14,color:'var(--muted)',marginBottom:4 }}>Name</div><div style={{ color:'var(--fg)',fontWeight:600 }}>{user.name}</div></div>
        <div style={{ marginBottom:16 }}><div style={{ fontSize:14,color:'var(--muted)',marginBottom:4 }}>Email</div><div style={{ color:'var(--fg)' }}>{user.email}</div></div>
        {user.linkedin && <div style={{ marginBottom:16 }}><div style={{ fontSize:14,color:'var(--muted)',marginBottom:4 }}>LinkedIn</div><a href={user.linkedin} target="_blank" style={{ color:'var(--gold)',fontSize:15 }}>{user.linkedin}</a></div>}
...
function AuthScreen({ onLogin }: { onLogin:(u:any)=>void }) {
  ...
  async function handleOtp() {
    ...
    try {
      ...
      const d = await res.json();
      if (!res.ok) {
        const msg = d.error || 'Invalid code. Please try again.';
        setError(msg); toast.error(msg);
      } else {
        toast.success(mode === 'login' ? 'Welcome back.' : 'Welcome to the community!');
        onLogin(d.user);
      }
    } catch {
      const msg = 'Network error. Please try again.';
              ...
              <>
                {[['Full name','name','text'],['Company','company','text'],['Role','role','text'],['Industry','industry','text'],['Team size','team_size','text']].map(([label,field,type])=>(
                  <div key={field} style={{ marginBottom:12 }}>
                    <div style={{ fontSize:14,color:'var(--muted)',marginBottom:4,fontWeight:600,textTransform:'none',letterSpacing:'0.01em' }}>{label}</div>
                    <input type={type} value={(form as any)[field]} onChange={e=>setForm(p=>({...p,[field]:e.target.value}))} style={{ width:'100%',background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,padding:'10px 14px',color:'var(--fg)',fontSize:16,outline:'none',boxSizing:'border-box' }} />
                  </div>
                ))}
                <div style={{ marginBottom:12 }}>
                  <div style={{ fontSize:14,color:'var(--muted)',marginBottom:4,fontWeight:600,textTransform:'none',letterSpacing:'0.01em' }}>LinkedIn URL *</div>
                  <input type="text" value={form.linkedin} onChange={e=>setForm(p=>({...p,linkedin:e.target.value}))} placeholder="https://linkedin.com/in/yourprofile" style={{ width:'100%',background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,padding:'10px 14px',color:'var(--fg)',fontSize:16,outline:'none',boxSizing:'border-box' }} />
                  <div style={{ fontSize:13,color:'var(--muted)',marginTop:4 }}>Required. Used instead of corporate email verification.</div>
                </div>
              </>
            )}
            <div style={{ marginBottom:16 }}>
              <div style={{ fontSize:14,color:'var(--muted)',marginBottom:4,fontWeight:600,textTransform:'none',letterSpacing:'0.01em' }}>Email address</div>
              <input type="email" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))} style={{ width:'100%',background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,padding:'10px 14px',color:'var(--fg)',fontSize:16,outline:'none',boxSizing:'border-box' }} />
            </div>
            {error && <div style={{ color:'var(--red)',fontSize:15,marginBottom:12 }}>{error}</div>}
            <button onClick={handleSubmit} disabled={loading} style={{ width:'100%',background:'var(--cream)',border:'none',borderRadius:10,padding:'12px',color:'var(--gold-btn-text)',fontWeight:700,fontSize:17,cursor:'pointer',marginBottom:16,opacity:loading?0.7:1 }}>{loading?'Sending…':'Send verification code'}</button>
            <div style={{ textAlign:'center',fontSize:15,color:'var(--muted)' }}>
              {mode==='login' ? <>No account? <button onClick={()=>{setMode('register');setError('');}} style={{ background:'none',border:'none',color:'var(--gold)',cursor:'pointer',fontWeight:600,fontSize:15 }}>Register</button></> : <>Have an account? <button onClick={()=>{setMode('login');setError('');}} style={{ background:'none',border:'none',color:'var(--gold)',cursor:'pointer',fontWeight:600,fontSize:15 }}>Sign in</button></>}
            </div>
          </>
        )}
        {step==='otp' && (
          <>
            <p style={{ color:'var(--muted)',fontSize:16,textAlign:'center',marginBottom:24 }}>Enter the 6-digit code sent to <strong style={{ color:'var(--fg)' }}>{form.email}</strong></p>
            ...
            {error && <div style={{ color:'var(--red)',fontSize:15,marginBottom:12,textAlign:'center' }}>{error}</div>}
            <button onClick={handleOtp} disabled={loading||otp.length!==6} style={{ width:'100%',background:'var(--cream)',border:'none',borderRadius:10,padding:'12px',color:'var(--gold-btn-text)',fontWeight:700,fontSize:17,cursor:'pointer',opacity:loading||otp.length!==6?0.7:1 }}>{loading?'Verifying…':'Verify & Sign in'}</button>
            <button onClick={()=>{setStep('form');setOtp('');setError('');}} style={{ width:'100%',background:'none',border:'none',color:'var(--muted)',cursor:'pointer',fontSize:15,marginTop:12 }}>← Back</button>
          </>
        )}
      </div>
    </div>
